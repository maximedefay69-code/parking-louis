...
# --- 3. INTERFACE & PRÉDICTION ---
st.title("🅿️ SLOT V65 - 15 Features")

col1, col2, col3 = st.columns([1, 2, 3])
num_v = col1.text_input("N°")
type_v = col2.selectbox("Type", ["Rue", "Boulevard", "Avenue", "Place", "Quai"])
    ...
    if nom_v:
        with st.spinner("Analyse..."):
            ...
            if 'features' in geo and len(geo['features']) > 0:
                ...
                # --- PRÉPARATION DU TABLEAU DE 15 FEATURES ---
                X_dict = {
                    ...
                    'RUE': nom_v.upper(),                       # 4
                    'VILLE': "Paris",                           # 5
                    'TRAFIC': 0.0,                              # 6 (Format décimal 0.0 - 1.0)
                    'PARKING % OC': 0.5,                        # 7
                ...
                # Forçage de l'ordre exact des 15 colonnes
                colonnes_15 = [
                ...
                # Panneau de contrôle pour Louis
                st.subheader("📊 Contrôle Terrain")
                c1, c2, c3, c4 = st.columns(4)
                c1.metric("Météo", mto_label)
                c2.metric("Temp", f"{temp_val}°C")
                c3.metric("Trafic", "0.0 (Fluide)")
                c4.metric("Places", total_p)
                ...
                try:
                    ...
                    st.divider()
                    st.success(f"🤖 IA : **{libres} places libres** estimées (Occupation: {round(occ_pred*100)}%)")
                except Exception as e:
                    st.error(f"Erreur modèle : {e}")
                    st.write("Vérifiez l'ordre des colonnes ci-dessous :")
                    st.dataframe(X_df)
